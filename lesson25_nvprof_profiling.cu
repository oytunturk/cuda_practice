...
void run_sum_array_1d(int argc, char** argv)
{
	...
	printf("Kernel launched with grid(%d,%d,%d) and block(%d,%d,%d) \n",
...
//Run with nvprof to profile different grid and block settings:
//!nvcc -link common.obj -o profiling.out hello_cuda.cu
//Simplest use of nvprof is without any options and program args
//!nvprof profiling.out
